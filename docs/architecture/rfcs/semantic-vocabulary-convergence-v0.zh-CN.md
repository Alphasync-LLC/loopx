...
- **一条备注必须写什么：** 什么条件产生这个值——运行时要成立什么，代码才会选它。
  ...
  因此一条只是复述自身名字的备注按失败处理，而不算覆盖。
- 当产生条件无法确定时，备注就如实写明，并点名什么证据可以了结它，形式为
  `Unresolved: … Missing evidence: …`。81 个中有 2 个处于此状态，且都没有臆测：
  `settlement_failure_kind.cancelled` 在两个 owner 中都有声明、解码器也接受，
  但 `loopx/` 下没有任何分支选它，只有伪造它的测试用到，且没有
...
- 本次的棘轮是一个新文件
  ...
  改测试即被覆盖；没有备注、备注为空、备注除自身标识符外不含任何词、以及未点名缺失
  证据的 unresolved 标记，都会失败；未解析数量被钉在 2，使 “unresolved” 无法成为
  省事的默认答案。另有一条测试会在某个词表被登记到两个棘轮都不走的层时失败。
- 本次未处理：备注是散文，只检查其实质、不检查其真伪。没有任何机制保证代码移动之后
  所述的产生条件仍然成立。对 `cross_runtime` 层而言并不存在可供比对的 producer
  扫描，这与 F1/F2 的值域边界已经披露的是同一个缺口。
