...
- **交付成熟度：** 设计阶段。隔离的合成视觉探索不证明生产执行或流式链路已通过验收。
...
| L1 真实小团队现场 | 既有团队执行入口展示有范围的工作、返回与准确证据；列表 / 场景一致 | 已准入绑定；真实入口的 V1/V2/V3/V6 | Collaboration 投影 + Chat/packaged UI；关闭场景 |
...
下一个可执行实现是 **L1**，完整的观测到用户 vertical，而不是单独一个 event
schema。同一交付计划包含源收集、typed 投影、重连语义、打包前端、独立 CLI
回读和失败用例。选择新 transport schema 前先盘点既有 receipt 身份，昂贵校验
离开常规轮询。
...
- **Presentation owner，L1 前：** 最终美术方向与 motion grammar。建议融合指挥台
  与空间研究工作室，以语义缩放、结论修订为特色；验证有内容与失败状态。
