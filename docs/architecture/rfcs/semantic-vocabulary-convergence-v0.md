...
- **I5 Ratchets only fall.** Retirement, twin, and inventory budgets may be
  lowered in any PR. Raising one requires maintainer approval recorded in
  Appendix B.
...
| Key | Content | Check |
| --- | --- | --- |
| `coverage_floor` | counts of vocabularies, owner symbols, literal-scan fields, projections, relations, schema versions; the scanned suffix set | Actual counts are at or above the floor; declared suffixes cover the floor set (I8) |
...
| `inventory_ratchets` | budgets for same-runtime fork names and definitions, conflicting names and definitions, schema-version forks | Inventory summary counts are at or below budget (I5) |
...
forks, and conflicting values, one entry per line. Consumer counts are printed
by `--report` and not committed, so an ordinary consumer edit does not touch
the file. Single-module string constants are counted, not listed.
