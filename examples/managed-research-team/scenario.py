...
def task(revision: str, question: str) -> str:
    return (
        ...
        "independent_source_families (integer), repost_stale (boolean), source_refs (list of source ids), "
...
def validate_worker(workspace: Path, revision: str) -> dict:
    ...
    if not isinstance(refs, list) or any(not isinstance(ref, str) for ref in refs) or not required.issubset(refs) or not result.get("reason"):
...
def validate_report(root: Path) -> dict:
    ...
    for worker in WORKERS:
        for revision in REVISIONS:
            ...
            entry = json.loads((root / "accepted" / (worker + "-" + revision + ".json")).read_text())
            output = validate_worker(root / worker / revision, revision)
            if entry.get("turn_status") != "committed" or entry.get("evidence") != output:
                raise ValueError("dependency_not_accepted")
            if dependencies.get(identity) != sha256(encoded(output)).hexdigest():
                raise ValueError("lead_did_not_adopt_dependency")
